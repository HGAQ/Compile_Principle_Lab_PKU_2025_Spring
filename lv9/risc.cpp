...
int get_alloc(const koopa_raw_type_t ty) {
	switch (ty->tag) {
	case KOOPA_RTT_UNIT:
		return 0;
	...
	case KOOPA_RTT_ARRAY:
		return ty->data.array.len * get_alloc(ty->data.array.base);
	default:
		assert(false);
	}
}
...
void Visit(const koopa_raw_function_t &func) {
    ...
    for (int i = 0; i < func->bbs.len; ++i)
    {
        ...
        for (size_t j = 0; j < bb->insts.len; ++j)
        {
            ...
            if (inst->kind.tag == KOOPA_RVT_ALLOC) {
				int size = get_alloc(inst->ty->data.pointer.base);
				add_arr_arg += size;
			}
        }
    ...
    |   | 0 |
       ...
    */
    stack_num += 4 + add_len_arg * 4 + add_arr_arg;
...
{  
    ...
    if (init->kind.tag == KOOPA_RVT_INTEGER){
        ...
        cout << "\t.zero " << get_alloc(init->ty) << std::endl;
    }
...
void Visit(const koopa_raw_aggregate_t& aggregate) {
	for (int i = 0; i < aggregate.elems.len; i++) {
		auto element = reinterpret_cast<koopa_raw_value_t>(aggregate.elems.buffer[i]);
		if (element->kind.tag == KOOPA_RVT_INTEGER) {
            cout << "\t.word " << element->kind.data.integer.value << endl;
		}
		else if(element->kind.tag == KOOPA_RVT_AGGREGATE){
			Visit(element);
		}
	}
}
...
void alloc(const koopa_raw_value_t& value) {
	int size = get_alloc(value->ty->data.pointer.base);
    Stack* curr_stack = REG.name2stk(REG.curr_stk); // get curr stack
	curr_stack->stk_val_set(value, size);
}
...
void Visit(const koopa_raw_get_ptr_t& get_ptr, const koopa_raw_value_t& value) {
    cout<<"\t#getptr"<<endl;
    Stack* curr_stack = REG.name2stk(REG.curr_stk); // get curr stack
    std::string base_reg = REG.reg_set(false); // allocate a reg for val of get ptr
    int offset = curr_stack -> val2offset(get_ptr.src);
	switch (get_ptr.src->kind.tag) {
	    case KOOPA_RVT_LOAD:
	    	Print_lw(offset, base_reg);
	    	break;
	    case KOOPA_RVT_GLOBAL_ALLOC:
            cout << "\tla " << base_reg << ", " << "glob_" << REG.pglobal_val -> val2idx(get_ptr.src) << "\n";
	    	break;
        default:
            break;
	}
    /////////////////////////////////////////////////////
    //i don't know
    std::string index_reg = REG.reg_set(false); // allocate a reg for val of get ptr
    if (get_ptr.index->kind.tag == KOOPA_RVT_INTEGER){   
        //keep it in reg
        std::cout << "\tli " << index_reg << ", " << get_ptr.index->kind.data.integer.value << std::endl;
    }
    else if(get_ptr.index->kind.tag == KOOPA_RVT_FUNC_ARG_REF){
        auto index = get_ptr.index->kind.data.func_arg_ref.index;
        std::string curr_regstr = "a" + std::to_string(index);
        if (index < 8){
            cout << "\tmv " << index_reg << ", " << curr_regstr << endl;
        }
        else{
            int curr_stack_size = curr_stack->stack_len;
            int offset = 4 * (index - 8);
            Print_lw(curr_stack_size + offset, index_reg); //do: load 4B in offset to reg  
        }
    }
    else{
        //save it from stack to reg
        int stack_offset = curr_stack->val2offset(get_ptr.index); // get offset of the value in stack
        Print_lw(stack_offset, index_reg); //do: load 4B in offset to reg  
    }
    /////////////////////////////////////////////////////////////////////////
    std::string step_reg = REG.reg_set(false); // allocate a reg for val of get ptr
    int step_size = get_alloc(get_ptr.src->ty->data.pointer.base);
    std::cout << "\tli " << step_reg << ", " << step_size << std::endl;
    std::cout << "\tmul " << index_reg << ", " << index_reg <<", " << step_reg <<std::endl;
    std::cout << "\tadd " << base_reg << ", " << base_reg <<", " << index_reg <<std::endl;

    int curr_stack_offset = curr_stack->stk_val_set(value);
	Print_sw(curr_stack_offset, base_reg);

    REG.reg_reset(REG.name2idx(step_reg));
    REG.reg_reset(REG.name2idx(index_reg));
    REG.reg_reset(REG.name2idx(base_reg));
}
...
void Visit(const koopa_raw_get_elem_ptr_t& get_elem_ptr, const koopa_raw_value_t& value) {
	...
	Stack* curr_stack = REG.name2stk(REG.curr_stk); // get curr stack
    std::string base_reg = REG.reg_set(false); // allocate a reg for val of get ptr
    int offset = curr_stack -> val2offset(get_elem_ptr.src);
	switch (get_elem_ptr.src->kind.tag) {
	    case KOOPA_RVT_LOAD:
	    	Print_lw(offset, base_reg);
	    	break;
	    case KOOPA_RVT_GLOBAL_ALLOC:
            cout << "\tla " << base_reg << ", " << "glob_" << REG.pglobal_val -> val2idx(get_elem_ptr.src) << "\n";
	    	break;
        case KOOPA_RVT_GET_PTR:
            Print_lw(offset, base_reg);
	    	break;
        case KOOPA_RVT_ALLOC:
            Print_addi(-offset, base_reg);
	    	break;
        case KOOPA_RVT_GET_ELEM_PTR:
		    Print_lw(offset, base_reg);
		    break;
        default:
            cerr<<get_elem_ptr.src->kind.tag<<endl;
            break;
	}
    /////////////////////////////////////////////////////
    //i don't know
    ...
    if (get_elem_ptr.index->kind.tag == KOOPA_RVT_INTEGER){   
        //keep it in reg
        std::cout << "\tli " << index_reg << ", " << get_elem_ptr.index->kind.data.integer.value << std::endl;
    }
    else if(get_elem_ptr.index->kind.tag == KOOPA_RVT_FUNC_ARG_REF){
        auto index = get_elem_ptr.index->kind.data.func_arg_ref.index;
        std::string curr_regstr = "a" + std::to_string(index);
        if (index < 8){
            cout << "\tmv " << index_reg << ", " << curr_regstr << endl;
        }
        ...
        //save it from stack to reg
        int stack_offset = curr_stack->val2offset(get_elem_ptr.index); // get offset of the value in stack
        Print_lw(stack_offset, index_reg); //do: load 4B in offset to reg  
    }
    /////////////////////////////////////////////////////////////////////////
    std::string step_reg = REG.reg_set(false); // allocate a reg for val of get ptr
    int step_size = get_alloc(get_elem_ptr.src->ty->data.pointer.base->data.array.base);
    std::cout << "\tli " << step_reg << ", " << step_size << std::endl;
    std::cout << "\tmul " << index_reg << ", " << index_reg <<", " << step_reg <<std::endl;
    std::cout << "\tadd " << base_reg << ", " << base_reg <<", " << index_reg <<std::endl;
...
{   
    ...
    if (load.src->kind.tag == KOOPA_RVT_GLOBAL_ALLOC){
        ...
        int curr_stack_offset = curr_stack -> val2offset(load.src);
		Print_lw(curr_stack_offset, curr_reg);
		Print_lw(0, curr_reg, curr_reg);
	}
...
void Visit(const koopa_raw_store_t &store, const koopa_raw_value_t &value)
{
    ...
    if (store.dest->kind.tag == KOOPA_RVT_GLOBAL_ALLOC){
        ...
        std::string temp_reg = REG.reg_set(value, false); // allocate a reg for val of store
        Print_lw(curr_stack -> val2offset(store.dest), temp_reg);//do: store 4B in reg to offset   
        Print_sw(0, curr_reg, temp_reg);//do: store 4B in reg to offset   
        REG.reg_reset(REG.name2idx(temp_reg)); // deallocate a reg for val of load
    }
...
void Print_addi(int stack_num_, std::string curr_reg)
{
    if (stack_num_ >= -BOUNDARY && stack_num_ < BOUNDARY){
        std::cout << "\taddi " << curr_reg << ", sp, " << - stack_num_ << std::endl; // arrange stack
    }
    else{
        int idx = REG.name2idx(REG.reg_set( false));
        std::cout << "\tli " << REG.idx2name(idx) << ", " << - stack_num_ << std::endl;
        //shen ren bug ! abbi -> add 
        std::cout << "\tadd " << curr_reg << ", sp, " << REG.idx2name(idx) << std::endl;
        REG.reg_reset(idx);
    }
}
