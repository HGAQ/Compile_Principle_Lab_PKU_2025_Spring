...
void Print_addi(int stack_num_, std::string curr_reg = "sp");
...
class Stack{
    public:
        ...
        int stk_val_set(const koopa_raw_value_t value, int size = 4){
            ...
            if(stack_offset_table.find(value) == stack_offset_table.end()){
                if(curr_stack_offset + size <= stack_len - 4)//del the ra save!
                {
                    stack_offset_table[value] = curr_stack_offset;
                    curr_stack_offset += size;
                    return curr_stack_offset - size;
                }
                return -1;
            }
            else{
